...
- **Nome do repositório:** `web-2026-1-SEUNOME` *(substituir pelo nome do usuário)*
- **Usuário GitHub:** *(a ser informado)*
- **Link do repositório:** *(a ser gerado após criação do repositório)*
...
- **Usuario** (id, matricula/siape, cpf, nome, email, perfil)
- **Agendamento** (id, discente_id, profissional_id, servico, status, data_hora, campus, modalidade, demanda_texto, triagem_ia)
- **Prontuario** (id, agendamento_id, evolucao, encaminhamento, retorno_agendado)
- **Encaminhamento** (id, prontuario_id, tipo, servico_destino, descricao)
- **DisponibilidadeAgenda** (id, profissional_id, dia_semana, hora_inicio, hora_fim, capacidade_maxima, campus)
- **Avaliacao** (id, agendamento_id, nota, comentario, data_envio)